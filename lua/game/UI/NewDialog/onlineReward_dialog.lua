...
function zhiyuan:initUI()
    self:setLayout("onlineReward_dialog.json")
    self:loadViewsTo()
    self.btn_close:setScriptCallback(ButtonHandler(display.closeDialog, self.priority))
    self.btn_get:setScriptCallback(ButtonHandler(self.OnRecieveClick, self))
    self:finRewardStrSet()

    ViewTemplates.setImplements(self.layout1, "LayoutImplement", {callback=Handler(self.onUpdateItemsCell1, self), withIdx=false})--当前奖励
    ViewTemplates.setImplements(self.layout2, "LayoutImplement", {callback=Handler(self.onUpdateItemsCell2, self), withIdx=false})--最终奖励

    self:update()
    RegTimeUpdate(self.lb_onlineTitle.view, function()
        self:update()
    end, 1)
end
...
function zhiyuan:finRewardStrSet()
    ...
    self.lb_finalReward:setString(finRewardStr)
end

function zhiyuan:OnRecieveClick()
    if self.receiveNum == #(SData.getData("olrwdt")) then
        self.lb_nextReceiveTip:setString(Localize("labelAllReceiveToday"))
    else
...
function zhiyuan:CurReward()--当前奖励
    ...
    self.lb_nextReceiveTip:setVisible(false)
    local num = self.context:getProperty(const.ProOnlineCount) --当前领取次数
    if num>=#SData.getData("olrwdt") then
        num = #SData.getData("olrwdt")-1
        self.lb_nextReceiveTip:setVisible(true)
        self.lb_nextReceiveTip:setString(Localize("labelOverTip"))
    end
    self.receiveNum = num
    ...
    self.layout1:setLayoutDatas(infosCurReward)
end
...
function zhiyuan:FinReward()--最终奖励
    ...
    self.layout2:setLayoutDatas(infosFinReward)
end
...
function zhiyuan:onUpdateItemsCell1(reuseCell, layout, item)
    ...
    reuseCell.lb_itemNum:setString(N2S(item.resNum))
    return reuseCell
end
...
function zhiyuan:onUpdateItemsCell2(reuseCell, layout, item)
    ...
    reuseCell.lb_itemNum2:setString(N2S(item.resNum))
    return reuseCell
end
...
function zhiyuan:update()
    ...
    if self.state ~= state then
        ...
        if state == 1 then
            ...
            self.lb_remainTime:setVisible(false)
        else
    ...
    if self.rtime and self.rtime > 0 then
        self.lb_remainTime:setVisible(true)
        self.lb_remainTime:setString(Localizef("nextReceiveTime", {time=Localizet(self.rtime)}))
    else
        self.lb_remainTime:setVisible(false)
    end
    self:finRewardStrSet()
