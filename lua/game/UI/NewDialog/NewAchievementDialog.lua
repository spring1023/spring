...
function AchievementDialog:initUI()
    ...
    --提示角标
    local btn_achieve = self.pageBtns[2]
    local redNum1 = GameUI.addRedNum(btn_achieve.view,s[1]-70,s[2]/2,0,1,10)
...
function AchievementDialog:initAchievementInfos()
    ...
    for i,v in ipairs(achieveType) do
        local id = v
        if ddata[id] then
            local info = {id=id, item = ddata[id], __order=(SData.getData("achieves", ddata[id].id).order or 0) +i}
...
function AchievementDialog:achievementCallCell(cell, scroll, info)   
    ...
    -- end
    local s = cell.img_achieveType:getContentSize()
    GameUI.addActiveImg(cell.img_achieveType,item.type+200,s[1],s[2],s[1]/2,s[2]/2,0)
    cell.lb_achieveTitle:setString(Localizef("dataAchieveName" .. i,{a = BU.getBuildName(item.tid), b = item.tlv}))

    if i == const.ActTypeBuildLevelUp then
    ...
    cell.img_achievePro:setProcess(true,item.glv/item.tlv)
    cell.progress:setVisible(item.glv < item.tlv)
    ...
    cell.btn_achieve:setVisible(item.glv >= item.tlv)
    cell.btn_achieve:setScriptCallback(ButtonHandler(function()
        if info.id == const.ActTypeRename and item.isget == 0 and item.glv < item.tlv then
    ...
    if item.isget == 0 then
        if info.id == const.ActTypeRename and item.glv < item.tlv then
            cell.progress:setVisible(false)
            cell.btn_achieve:setVisible(true)
            cell.lb_get:setString(Localize("buttonGo"))
        elseif item.glv>=item.tlv then
            ...
            --未完成
            cell.progress:setVisible(true)
            cell.btn_achieve:setVisible(false)
        end
    else
        cell.progress:setVisible(false)
        cell.btn_achieve:setVisible(true)
        cell.lb_get:setString(Localize("labelAlreadyReceive"))
        cell.btn_achieve:setGray(true)
        cell.btn_achieve:setEnable(false)
    end
    return cell
end
    ...
    _G["GameNetwork"].request("getachieve",{getachieve = {id}},function(isSuc, data)
        ...
        if isSuc then
            ...
            if self.achievement then
                ...
                self.achievementLoaded = nil
                self.view:runAction(ui.action.sequence({{"delay",1},{"call",unlock}}))
