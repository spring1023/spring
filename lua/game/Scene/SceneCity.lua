local SceneCity = {state="cleanup", delta=0}

function SceneCity:loadGround(scene)
    local map = scene.map
    ...
    --local land = ui.sprite("images/background/sceneBgCity.png",{sizeX, sizeY}, false, true)
    local land = ui.spriteBlock("images/background/sceneBgCity.png",{sizeX, sizeY},{4,3}, false, true)
    display.adapt(land,0,0)
    ground:addChild(land)
    scene.bgPng = "images/background/sceneBgCity.png"
    local config = GMethod.loadConfig("configs/sceneScale.json").SceneCity
    scene.scroll:moveAndScaleToCenter(smin*config, sizeX/2, sizeY/2,0.5)
    UIeffectsManage:showEffect_jiangshiCJ(ground,sizeX/2,sizeY/2)

    scene.bottomType = "City"
    return self.view
end
