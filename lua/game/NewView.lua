...
function View:setSize(sx, sy)
    self.view:setContentSize(cc.size(sx, sy))
    self.size = {sx, sy}
    self:onSizeChange()
end
...
function View:_addViews(views, toView)
    ...
    elseif views then
        ...
        for _, vsetting in ipairs (views) do
            ...
            if view then
                if vsetting.id then
                    self:_setView({vid=vsetting.id,view=view})
                end
                if vsetting.parent then
                ...
                -- 增加一种特殊适配模式，直接改用全屏适配
                if vsetting.suitable then
                    display.adapt(view.view, 0, 0, GConst.Anchor.Center, {scaleType=GConst.Scale[vsetting.suitable]})
                    if vsetting.autoSuitable then
                        local baseScale = view.view:getScaleX()
                        view:setSize(display.winSize[1] / baseScale, display.winSize[2] / baseScale)
                    end
                end
                if vsetting.views then
                    self:_addViews(vsetting.views, view)
...
local __newViewScale
function View:getUIScale()
    if not __newViewScale then
        local _winSize = display.winSize
        local scale = 1
        if _winSize[2]*1024 >= _winSize[1]*640 then
            scale = _winSize[1]/2048
        else
            scale = _winSize[2]/1280
        end
        __newViewScale = scale
    end
    return __newViewScale
end
...
function ViewNode:setSValue(svalue)
    if self._templateView and self._templateView.setSValue then
        self._templateView:setSValue(svalue)
    elseif self.view.setSValue then
        self.view:setSValue(svalue)
    end
end
...
function ViewScroll:onCreate()
    ...
    if scroll.elastic then
        view:setElastic(scroll.elastic)
    end
...
function ViewScroll:scrollBy(x,y,speed)
...
function ViewScroll:ajustPosition(cell,speed,topLen,botLen)
    if not cell then
        if self._isX then
        ...
        if not self._isX then
            ...
            if nodePos.y<botLen then
                self:scrollBy(0,botLen-nodePos.y,speed)
            elseif nodePos.y>size[2]-topLen then
                self:scrollBy(0,size[2]-topLen-nodePos.y,speed)
            end
        else
            ...
            if nodePos.x<botLen then
                self:scrollBy(botLen-nodePos.x,0,speed)
            elseif nodePos.x>size[1]-topLen then
                self:scrollBy(size[1]-topLen-nodePos.x,0,speed)
            end
        end
    end
end
...
function ViewScroll:locationIndex(index,speed)
    if self._reuseMode then
        ...
        if children[1] then
            ...
            if dx then
                if dx == 0 then
                    x = p + epx + (realX-1) * (cellSize[1] + off[1]) + cellSize[1] / 2
                else
                    x = mSize[1] - p - epx - (realX-1) * (cellSize[1] + off[1]) - cellSize[1] / 2
                end
            else
                x = 0
            end
            if dy then
                if dy == 0 then
                    y = p + epy + (realY-1) * (cellSize[2] + off[2]) + cellSize[2] / 2
                else
                    y = mSize[2] - p - epy - (realY-1) * (cellSize[2] + off[2]) - cellSize[2] / 2
                end
            else
                y = 0
            end
            self.view:moveAndScaleToCenter(1, x, y, 0)
        end
        return
    end
...
function ViewScroll:setLazyTableData(infos, callback, reuseMode, reuseSkip, refreshOnly)
    ...
    if not self._reuseChilds then
        ...
        if self._reuseMode > 0 and (self._infoLength <= #(self._reuseChilds) or self._reuseMode <= #(self._reuseChilds) and self._reuseSkip <= #(self._noreuseChilds)) then
            ...
            self:onEvent("scrollTo", oldCenter[1] - self.size[1] / 2, oldCenter[2] - self.size[2] / 2)
...
function ViewScroll:setBusyTableData(infos, callback)
    local index = 1
    self._infoLength = KTLen(infos)
    if not self._reuseChilds then
        self._reuseChilds = {}
    ...
    while index <= self._infoLength do
        local info = infos[index]
        local item = self._reuseChilds[index]
        if item then
            callback(item, self, info, index)
            item._realIdx = index
        else
            item = callback(false, self, info, index)
            item._realIdx = index
            self:addChild(item)
            self._reuseChilds[index] = item
        end
        index = index + 1
    end
    self._currentStartIdx = 1
...
function ViewLabel:onCreate()
    ...
    if not self._createrConfig then
        ...
        if overflow == 2 then
            ...
            -- 可以自动缩小的适配方式
            params.fontW = params.width
            params.fontH = params.height
        else
            if overflow == 0 then
                width = 0
                params.width = 0
        ...
        if setting.outlineColor then
            ...
            self.view:setShadowColor(cc.c4b(cctab[1],cctab[2],cctab[3],cctab[4] or 255))
        end
    end
    ...
    if not self._createrConfig or overflow == 1 then
        if limit[2]>0 and limit[2]<size[2] then
            ...
            if not self._inScroll then
                local scroll = ui.scrollNode(limit, self._setting.inScroll and -1 or 4, false, true, {scroll=true, clip=true, rect={0, 0, limit[1], size[2]}})
                ...
                -- self.view:setContentSize(cc.size(size[1], size[2]))
...
function ViewImage:resetImage()
    ...
    if setting.s9edge and ( ((type(setting.s9edge) == "number") and (setting.s9edge>0)) or (type(setting.s9edge) == "table") ) then
        self._isScale9 = true
        local scale9 = setting.s9edge
        self.view = ui.scale9(setting.image, setting.s9edge, size)
    else
        self._isScale9 = false
        local _realImg = setting.image
        if _realImg:find("%[language%]") then
            local tmp = _realImg:gsub("%[language%]", General.language)
            if not cc.FileUtils:getInstance():isFileExist(tmp) then
                tmp = _realImg:gsub("%[language%]", "EN")
            end
            _realImg = tmp
        end
        if _viewTemplateFunc then
            self.view = _viewTemplateFunc("image", _realImg, size)
        else
            self.view = ui.sprite(_realImg, size, nil, setting.lazy)
        end
        if not self.view then
            if setting.repeatMode then
                self.view = ui.sprite(_realImg)
            ...
            if not self.view then
                self.view = ui.sprite("images/bgWhiteEdgeWhite.9.png", size)
            end
        end
        if setting.flip then
            if setting.flip=="X" then
...
function ViewImage:setImage(img, edge, sx, sy, keepScale)
    local setting = self._setting
    --预留接口改变图片文字语言
    if setting.language and General.language ~= "CN" then
        local endIdx = string.find(img,".png")
        img = string.sub(img,1,endIdx).."_"..General.language..".png"
    end
...
function ViewLayout:setLayout(configName)
    ...
    -- print("loadconfig:",configDir .. configName)
    local rawView
    if config then
...
function ViewLayout:setView(view, size, onLife)
    self.size = size
    self.view = view
    self._layouts = {}
    if onLife then
...
function ViewLayout:addLayout(configName, toView)
    ...
    if config then
        ...
        if toView then
            ...
            newLayout._createrConfig = config.createrConfig
